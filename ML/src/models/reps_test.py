...
def detect_peaks(dataset, cutoff=0.4, order=10, column="acc_r"):
    if len(dataset) <= 33:  # Or any other minimum length determined by your filter configuration
        print(f"Dataset too short for filtering: {len(dataset)} points")
        return 0
    ...
    plt.title(f"Peaks detected for {exercise}: {len(peaks)}Reps")
    plt.show()
...
for s in rep_df["set"].unique():
    ...
    reps = detect_peaks(subset, cutoff=cutoff, column=column)
    rep_df.loc[rep_df["set"] == s, "reps_predicted"] = reps
    rep_df.to_csv("reps_predicted.csv")
